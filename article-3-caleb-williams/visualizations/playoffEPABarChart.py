...
def create_epa_bar_chart(output_path='../output/visuals/epa_bar_chart.png'):
    ...
    # Add delta indicator between bars
    mid_x = (x_positions[0] + x_positions[1]) / 2
    mid_y = (regular_season_epa + playoff_epa) / 2
    
    # Arrow pointing up
    ax.annotate('', xy=(mid_x, playoff_epa - 0.01), 
               xytext=(mid_x, regular_season_epa + 0.01),
               arrowprops=dict(arrowstyle='->', lw=3, color='#27ae60'))
    
    # Delta text
    ax.text(mid_x, mid_y + 0.02, f'+{delta:.3f} EPA',
           ha='center', fontsize=14, fontweight='bold', color='#27ae60',
           bbox=dict(boxstyle='round,pad=0.5', facecolor='white', 
                    edgecolor='#27ae60', linewidth=2))
    
    ax.text(mid_x, mid_y - 0.02, f'(+{percent_improvement:.0f}% improvement)',
